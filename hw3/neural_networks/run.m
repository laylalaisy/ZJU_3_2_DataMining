load digit_data
[~, num_cases] = size(X);                      % 400 * 5000
train_num_cases = num_cases / 5 * 4;           % 5000 / 5 * 4 = 4000
X = reshape(X, [400, num_cases]);              % 400 * 5000
X = permute(X, [2, 1]);                        % 5000 * 400    

% X has the shape of [number of samples, number of pixels]
train_data = X(1:train_num_cases, :, :, :);    % 4000 * 400
train_label = y(:, 1:train_num_cases);         % 1 * 4000
test_data = X(train_num_cases+1:num_cases, :, :, :);    % 1000 * 400
test_label = y(:, train_num_cases+1:num_cases);         % 1 * 1000

weights.fully1_weight = randn([400, 25]) ./ 400;        % 400 * 25
weights.fully1_bias = rand([25, 1]);                    % 25 * 1
weights.fully2_weight = randn([25, 10]) ./ 25;          % 25 * 10
weights.fully2_bias = rand([10, 1]);                    % 10 * 1

% Training
fully1_weight_inc = zeros(size(weights.fully1_weight)); 
fully1_bias_inc = zeros(size(weights.fully1_bias));
fully2_weight_inc = zeros(size(weights.fully2_weight)); 
fully2_bias_inc = zeros(size(weights.fully2_bias));
...
for epoch = 1:max_epoch
    for i = 1:ceil(train_num_cases/batch_size)
        data = train_data((i-1)*batch_size+1:min(i*batch_size, train_num_cases), :, :, :); % 100 * 400
        label = train_label(:, (i-1)*batch_size+1:min(i*batch_size, train_num_cases));     % 1 * 400
